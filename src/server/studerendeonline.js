...
async function getJob (url) {
  ...
  await page.goto(url);
    ...
    Array.from(document.querySelectorAll(".container-fluid > .job-item-container")).map(item => ({
        ...
        "Tekst": item.querySelector('.job-content > .job-description').innerText || item.querySelector('.job-content > .job-teaser').innerText,
        "Link": item.querySelector('a').href,
        "Billede": item.querySelector('a > div > div > img').src
    })) 
    )
...
return jobs
 ...
 const nextUrl = `https://studerendeonline.dk/job/?&page=${nextPageNumber}`;
 return jobs.concat(await getJob(nextUrl))
}
};
const browser = await puppeteer.launch();
const firstUrl = "https://studerendeonline.dk/job/?&page=1"
const jobListe = await getJob(firstUrl);
const success = setAsync('studerendeonline', JSON.stringify(jobListe));
