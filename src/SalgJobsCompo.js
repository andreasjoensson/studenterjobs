...
import SalgJobs from "./SalgJobs";

const SalgJob = ({søgning}) => {
...
useEffect(() => {
  async function fetchData (){
    const resp = await fetch("http://localhost:3003/marketing");
...
return(
<>
<SalgJobs jobs={nuværendeJobs} siderPerSide={jobsPerSide} totalSider={job.length} paginate={paginateNye}/>
<Pagination page={currentPage} onChange={handleChange} count={Math.round(nytJob.length/jobsPerSide)}/>
</>
)
}

export default SalgJob