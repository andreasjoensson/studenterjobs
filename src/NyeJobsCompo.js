import NyeJobs from "./NyeJobs";
import React,{useState, useEffect} from 'react';
import {Pagination} from '@material-ui/lab';

const NytJob = ({søgning}) => {
...
useEffect(() => {
  async function fetchData (){
    const resp = await fetch("http://localhost:3003/api");
...
return(
<>
<NyeJobs jobs={nuværendeJobs} siderPerSide={jobsPerSide} totalSider={job.length} paginate={paginateNye}/>
<Pagination page={currentPage} onChange={handleChange} count={Math.round(nytJob.length/jobsPerSide)}/>
</>
)
}

export default NytJob